...
class CreateForm extends Component {
  ...
  onCreate = async (item, type) => {
    ...
    if (type === "category") {
      ...
      const res = await post_request("products/create_product", item);
    } else if (type === "ingredient") {
      const res = await post_request("ingredients/create_ingredient", item);
    } else if (type === "choice") {
      const res = await post_request(
        "product_choices/new_product_choice",
        item
      );
    }
...
const CategoryForm = ({ isUpdate, item, onCreate, onUpdate }) => {
  ...
  function onFileChange(e) {
    ...
    document.getElementById("img-preview").src = category.image.name;
        ...
        <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
          <div className="space-y-1 text-center">
            <svg
              ...
              aria-hidden="true"
            >
              <img id="img-preview" alt="img-preview" src={category.image} />
            </svg>
